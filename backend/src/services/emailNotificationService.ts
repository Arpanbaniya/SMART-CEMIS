            ...
            <div class="content">
              ...
              <div style="background: #f0f7ff; padding: 15px; border-radius: 4px; margin: 20px 0;">
                ...
                <ul style="margin: 10px 0; padding-left: 20px;">
                  <li><strong>Date:</strong> ${formatDate(new Date(event.date))}</li>
...
export async function sendRegistrationConfirmationEmail(
  ...
  try {
    ...
    if (userEmail) console.error('   To:', userEmail);
...
export async function sendWinnerCertificateEmail(
  winnerId: string,
  eventId: string,
  teamId?: string
): Promise<void> {
...
export async function sendFinalRoundNotificationEmail(
  teamId: string,
  eventId: string
): Promise<void> {
  try {
    ...
    if (!team || !event || !team.members) return;
...
export async function sendEventCompletionReminderEmail(eventId: string): Promise<void> {
  try {
    const event = await Event.findById(eventId);
    if (!event) return;
...
export async function sendNoticeEmailToAllUsers(
  noticeTitle: string,
