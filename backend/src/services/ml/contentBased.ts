...
export async function getContentBasedRecommendations(
  ...
  try {
    ...
    // Get user vector
    const userVectorData = await createUserVector(userId);
    ...
    // Calculate scores
    const scores: ContentBasedScore[] = [];
    ...
    for (const event of allEvents) {
      ...
      try {
        const eventVectorData = await createEventVector(eventId);
    ...
    console.log(
      `[ContentBased] Generated ${topRecommendations.length} recommendations for ${userId}`
    );
 ...
 */
export async function evaluateContentBasedModel(
  testUserIds: string[],
  trainUserIds: string[]
  ...
  try {
    ...
    const metrics: ContentBasedMetrics = {
