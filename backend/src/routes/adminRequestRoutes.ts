...
import { logAdminRequestApproval, logAdminRequestRejection, extractRequestMetadata } from '../utils/logger';
...
router.post('/requests', requireAuth, async (req, res) => {
  try {
    ...
    if (user && user.role === 'student_admin') {
      const unusedApprovedRequestsCount = await AdminRequest.countDocuments({
        userId: req.session.userId,
        status: 'approved',
...
router.patch('/requests/:id/reject', requireSuperAdmin, async (req, res) => {
  try {
    ...
    // Log rejection
    await logAdminRequestRejection(
      req.session.userId!,
      request._id.toString(),
      user?.email || 'Unknown',
...
router.get('/student-admin/status', requireAuth, async (req, res) => {
  try {
    const user = await User.findById(req.session.userId);
    ...
    const approvedRequest = await AdminRequest.findOne({
      userId: req.session.userId,
      status: 'approved'
    });
    ...
    let eventCount = 0;
    if (isStudentAdmin) {
      eventCount = await Event.countDocuments({ createdById: req.session.userId });
    }

    // Count approved requests for this user
    const approvedRequestsCount = await AdminRequest.countDocuments({
      userId: req.session.userId,
      status: 'approved'
    });
