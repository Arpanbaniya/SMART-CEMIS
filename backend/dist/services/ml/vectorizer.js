...
exports.createUserVector = createUserVector;
exports.createEventVector = createEventVector;
exports.getUserEventInteractions = getUserEventInteractions;
...
async function createUserVector(userId) {
    try {
        ...
        const { Favorite } = await Promise.resolve().then(() => __importStar(require('../../models/Favorite')));
        ...
        const { Feedback } = await Promise.resolve().then(() => __importStar(require('../../models/Feedback')));
        const feedbacks = await Feedback.find({ userId });
        if (feedbacks.length > 0) {
            const avgRating = feedbacks.reduce((sum, f) => sum + (f.rating || 3), 0) / feedbacks.length;
        ...
        return {
            ...
            metadata: {
                ...
                avgFeedbackRating: feedbacks.length > 0
                    ? feedbacks.reduce((sum, f) => sum + (f.rating || 3), 0) / feedbacks.length
                    : 3.0,
            },
        };
    }
 ...
 */
async function createEventVector(eventId) {
    try {
 ...
 * Comment: 0.4
 */
async function getUserEventInteractions(userId) {
    const { Registration } = await Promise.resolve().then(() => __importStar(require('../../models/Registration')));
    const { Favorite } = await Promise.resolve().then(() => __importStar(require('../../models/Favorite')));
    const { Feedback } = await Promise.resolve().then(() => __importStar(require('../../models/Feedback')));
