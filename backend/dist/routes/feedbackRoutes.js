...
router.get('/feedback/:eventId', async (req, res) => {
    try {
        ...
        const feedback = await Feedback_1.Feedback.find({ eventId })
...
router.post('/feedback/:eventId', requireAuth_1.requireAuth, async (req, res) => {
    try {
        const { eventId } = req.params;
        ...
        const existingFeedback = await Feedback_1.Feedback.findOne({ userId, eventId });
        if (existingFeedback) {
        ...
        try {
            (0, server_1.broadcastEventUpdate)(eventId, {
                type: 'feedback',
                feedbackCount: 1,
...
router.put('/feedback/:feedbackId', requireAuth_1.requireAuth, async (req, res) => {
    try {
        ...
        const event = await Event_1.Event.findById(feedback.eventId);
        // Broadcast feedback update for real-time sentiment analysis
        try {
            (0, server_1.broadcastEventUpdate)(feedback.eventId.toString(), {
...
router.delete('/feedback/:feedbackId', requireAuth_1.requireAuth, async (req, res) => {
    try {
        const { feedbackId } = req.params;
        const userId = req.session.userId;
        const feedback = await Feedback_1.Feedback.findById(feedbackId);
        ...
        const event = await Event_1.Event.findById(feedback.eventId);
        await Feedback_1.Feedback.findByIdAndDelete(feedbackId);
