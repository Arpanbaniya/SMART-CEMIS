...
router.get('/:eventId/comments', async (req, res) => {
    try {
        ...
        const comments = await Comment_1.Comment.find({ eventId })
...
router.get('/:eventId/user-comments', requireAuth_1.requireAuth, async (req, res) => {
    try {
        const { eventId } = req.params;
        const userId = req.session.userId;
        const comments = await Comment_1.Comment.find({ userId, eventId })
            .sort({ createdAt: -1 });
        res.json(comments.map(c => c.toJSON()));
    }
    catch (error) {
...
router.post('/:eventId/comments', requireAuth_1.requireAuth, async (req, res) => {
    try {
        const { eventId } = req.params;
        const userId = req.session.userId;
        ...
        const commentCount = await Comment_1.Comment.countDocuments({ userId, eventId });
        ...
        console.log('Total comments for event:', await Comment_1.Comment.countDocuments({ eventId }));
...
router.put('/comments/:commentId', requireAuth_1.requireAuth, async (req, res) => {
    try {
...
router.delete('/comments/:commentId', requireAuth_1.requireAuth, async (req, res) => {
    try {
        const { commentId } = req.params;
        const userId = req.session.userId;
        const comment = await Comment_1.Comment.findById(commentId);
        ...
        if (isSuperAdmin) {
            ...
            if (isCommentOwner) {
                ...
                const event = await Event_1.Event.findById(comment.eventId);
