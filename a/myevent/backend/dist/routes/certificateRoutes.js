...
router.post('/send-certificate', requireAuth_1.requireAuth, requireAuth_1.requireAdmin, async (req, res) => {
    try {
        ...
        if (isTeam) {
            ...
            for (const teamName of winnerIds) {
                ...
                for (const registration of teamRegistrations) {
                    const user = await User_1.User.findOne({ id: registration.userId });
            ...
            for (const winnerId of winnerIds) {
                const user = await User_1.User.findOne({ id: winnerId });
                if (user && user.email && user.firstName && user.lastName) {
...
async function generateCertificate(eventTitle, winnerName, firstName, lastName, isTeam) {
...
async function sendCertificateEmail(email, eventTitle, winnerName, certificateBuffer) {
