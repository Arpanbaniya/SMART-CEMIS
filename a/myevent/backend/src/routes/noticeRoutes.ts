...
import { sendNoticeEmailToAllUsers } from '../services/emailNotificationService';
...
router.post('/:id/send-email', requireAuth, requireSuperAdmin, async (req, res) => {
  try {
    const { id } = req.params;
    ...
    try {
      ...
      const result = await sendNoticeEmailToAllUsers(notice.title, notice.content, users);
