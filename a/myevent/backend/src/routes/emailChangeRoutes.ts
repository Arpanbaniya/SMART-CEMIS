...
import { sendEmailChangeVerification, sendEmailChangeNotification } from '../utils/sendgrid';
...
router.post('/request', requireAuth, async (req: any, res: Response) => {
  try {
    ...
    if (user.lastEmailChangeRequest) {
      ...
      const changesInLast24Hours = await User.countDocuments({
        _id: userId,
    ...
    const emailSent = await sendEmailChangeVerification(normalizedEmail, token, user.firstName || 'User');
...
router.post('/verify', async (req: any, res: Response) => {
  try {
    ...
    try {
      await sendEmailChangeNotification(newEmail, oldEmail, user.firstName || 'User');
      // Also notify old email (in case account was compromised)
      await sendEmailChangeNotification(oldEmail, oldEmail, user.firstName || 'User');
    } catch (emailError) {
...
router.post('/resend', requireAuth, async (req: any, res: Response) => {
  try {
    const userId = req.session.userId;
    ...
    const resendCount = (user as any).resendCount || 0;
    if (resendCount >= 5) {
    ...
    (user as any).resendCount = resendCount + 1;
    await user.save();

    // Send verification email
    try {
      await sendEmailChangeVerification(user.pendingEmail, newToken, user.firstName || 'User');
    } catch (emailError) {
