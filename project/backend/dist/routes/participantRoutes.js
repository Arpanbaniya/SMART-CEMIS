...
router.get('/', requireAuth_1.requireAuth, requireAdmin, async (req, res) => {
    try {
        ...
        if (user.role === 'super_admin') {
            if (eventId && typeof eventId === 'string') {
                query.eventId = new mongoose_1.default.Types.ObjectId(eventId);
            }
        }
        // If student admin, only get participants for their own events
            ...
            const userEvents = await Event_1.Event.find({ createdById: user._id });
        ...
        const registrations = await Registration_1.Registration.find(query)
            .populate('userId', 'firstName lastName email role')
            .populate('eventId', 'title date location category createdById')
            .sort({ registeredAt: -1 });
        // Log the access
        await (0, logger_1.createAdminLog)({
            ...
            requestMetadata: (0, logger_1.extractRequestMetadata)(req)
        });
...
router.delete('/', requireAuth_1.requireAuth, requireAdmin, async (req, res) => {
    try {
        ...
        const registrationsToRemove = await Registration_1.Registration.find({ _id: { $in: participantIds } })
        ...
        const result = await Registration_1.Registration.deleteMany({
            _id: { $in: participantIds }
        });
        // Update participant counts for affected events
        const affectedEventIds = registrationsToRemove.map(reg => reg.eventId._id);
        await Event_1.Event.updateMany({ _id: { $in: affectedEventIds } }, { $inc: { participantCount: -result.deletedCount } });
        // Log the action
        await (0, logger_1.createAdminLog)({
            userId: req.session.userId,
