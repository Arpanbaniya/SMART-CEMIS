...
router.get('/api/admin/logs', requireAuth_1.requireAuth, async (req, res) => {
    try {
        ...
        const { page = '1', limit = '50', timeframe, entityType, action, search } = req.query;
        ...
        const logs = await AdminLog_1.AdminLog.find(filter)
            .populate('userId', 'firstName lastName email')
            .sort({ createdAt: -1 })
            .skip(skip)
            .limit(limitNum);
        const total = await AdminLog_1.AdminLog.countDocuments(filter);
        res.json({
...
router.delete('/api/admin/logs', requireAuth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.session.userId;
        const user = await User_1.User.findById(userId);
        ...
        const result = await AdminLog_1.AdminLog.deleteMany(filter);
        res.json({
...
//# sourceMappingURL=adminLogRoutes.js.map