...
router.post('/register', async (req, res) => {
    try {
        ...
        const { raw, hashed } = (0, generateToken_1.generateVerificationToken)();
        ...
        (0, sendgrid_1.sendVerificationEmail)(email, raw).catch((err) => {
...
// │        GET CURRENT USER      │
// └──────────────────────────────┘
router.get('/user', requireAuth_1.requireAuth, async (req, res) => {
    try {
        ...
        const user = await User_1.User.findById(req.session.userId).select('-password');
